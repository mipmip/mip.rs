            ...
            Ok(Ok(event)) => {
                if !event.paths.is_empty() {
                    ...
                    if teststr.contains(&current_file) {
                        mip::markdown::to_html(&current_file, &temp_dir, port, show_frontmatter, &theme_class);
                    }
                }
            }
...
fn main() {
    ...
    if let Some(available_port) = get_available_port() {
        ...
        mip::view::window(available_port, temp_dir, show_frontmatter, theme, &path_file_for_view, runcmd_string.as_deref(), sidetoc_width, &sidetoc_position, keybinding_registry, paragraph_numbers, paragraph_numbers_start, cfg.history_size(), new_file_tx);
    }
    else{
        panic!("E2");
    }
